...
export default {
  ...
  async created() {
    try {
      ...
      this.datastoreList = data.result.records
    } catch (error) {
      console.error(error)
    }
  },
  computed: {
    datastoresFilter() {
      return this.datastoreList.filter(item => item.Name.match(this.cacheSearch))
    }
  },
  watch: {
    cacheArea() {
      let index = this.browseLog.find(item => item.Name === this.cacheArea.Name)
      if (index == undefined) {
